...
int main() {
	...
	ALLEGRO_FONT *font = al_load_ttf_font("Boom.ttf", 28, 0);
	if (font == NULL)
		cout << "font is null" << endl;
	//set up event queue items for event queue to catch//
	sample = al_load_sample("Future.wav");
	...
	while (!doexit) { //game loop
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			if (gotGoal == true)
				grid[goal_x][goal_y] = 0;
			al_draw_textf(font, al_map_rgb(255, 255, 255), 100, 200, ALLEGRO_ALIGN_CENTRE, "score = %i", score);
			al_flip_display();
		}// end render
